...
- インサイト取得は1投稿につき最大2回、1回の実行につき最大5投稿です。
- 認証情報、画像URL、品質ゲートに問題がある場合はAPIを呼ぶ前に停止します。

この設計はAPI消費と二重投稿を抑えることを優先しています。投稿失敗案を再送する場合は、
Threads側に投稿済みでないことを手動確認してからDB状態を戻してください。
